...
mysql = mysqlutil.MySQLUtil()
mysql.connect_db()

conn = m.connect(host='na05-sql.pebblehost.com', port=3306, user='customer_197460_starbase',
                 passwd='$$f9S#1kAgi1fQJcf47b', db='customer_197460_starbase')
cur = conn.cursor()
...
class Points:
    ...
    def get_data(self):
        cur.execute('select * from jsondata where id = 1;')
        self.data_sql = cur.fetchall()
        self.data_sql = self.data_sql[0][1]
        self.data = json.loads(self.data_sql)

    def dump_data(self):
        self.data_str = json.dumps(self.data)
        self.data_str = escape_string(self.data_str)
        sql = "UPDATE jsondata SET json = ('" + self.data_str + "') WHERE id = '1'"
        mysql.exec_sql(sql)
    ...
    def reset_guild(self, guild: discord.Guild):
        ...
        for member in self.data["guilds"][str(guild.id)]["members"]:
            self.data["guilds"][str(guild.id)]["members"][member]["points"] = 0
